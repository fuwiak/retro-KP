...
function setCrmStatus(message = "", tone = "info") {
  if (!els.crmStatus) return;
  const styles = getComputedStyle(document.documentElement);
  const accent = styles.getPropertyValue("--ui-color") || "rgb(255,0,0)";
  const palette = {
    info: accent.trim(),
  ...
  els.crmStatus.style.color = palette[tone] || palette.info;
}
...
async function refreshCrmInbox() {
  ...
  setCrmStatus("Загрузка писем...", "info");
  try {
    ...
    let newIndex = null;
    if (previousId) {
      const foundIndex = emails.findIndex((email) => email.id === previousId);
      if (foundIndex !== -1) {
        newIndex = foundIndex;
      }
    }
    ...
    if (emails.length) {
      ...
      log(`📬 Загружено ${emails.length} писем из IMAP`);
    } else {
...
async function handleCrmClassification() {
  ...
  setCrmStatus("AI анализирует письмо...", "info");
  try {
...
async function handleCrmProposal() {
  ...
  setCrmStatus("Генерируем КП...", "info");
  try {
...
async function handleCrmSend() {
  ...
  setCrmStatus("Отправляем данные в amoCRM...", "info");
